...
**多頁（Frame）介面的取捨：** 畫布一次只編輯／預覽「目前作用中的一頁」（左側「頁面」清單點列切換），
不是把所有頁面依「橫向＝不同段連續紙、縱向＝同段連續紙接續」2D 排開同時顯示——2D 版面在窄螢幕本來就要
退回簡單清單，考量到 2D 拖曳／縮放整層互動的複雜度與可驗證性，v1 版本直接統一採用這種較單純但功能完整
的分頁式介面；頁面清單本身有清楚標示每頁的切紙開關，多頁列印／匯出（PDF、ESC/POS）都會依序處理全部頁面。
