...
- **版面編輯器**：文字（橫書／直書）、圖片、圖文段落（單圖靠左／右，文字繞圖排列）、間隔、分隔線、條碼／QR Code、多欄（Row/Column，欄寬可自訂比例、可調欄距，不限兩欄）、群組
- **畫布直接操作**：多欄可在畫布上用欄位的「＋」對半分割、雙擊分隔線合併；圖片可拖曳側邊或下角把手縮放（下角按住 Shift 自由拉伸）；欄寬與高度把手即時重繪
- **編輯輔助**：復原／重做、複製貼上、多選與群組、鍵盤微調順序與欄寬，快捷鍵列在使用說明；「版面結構」可拖曳換層。
...
- **佔位變數／Mail Merge**：文字或圖片來源可用 `{{變數名稱}}`，貼上 JSON 陣列或 CSV 即可一次套用多筆資料，批次匯出多頁 PDF（一筆資料一頁）
...
- **`.ptan` 專案檔**：版本化 JSON 格式（有群組、圖文段落、直書或內嵌字體時為 v2，否則維持 v1 讓舊版也能開），可匯出／匯入，格式升級有 migration 機制；匯出可選擇內嵌用到的等寬開源字體
...
6. 於「批次資料」貼上 JSON 陣列或 CSV（首列為變數名稱）即可用「套用批次資料匯出 PDF」一次輸出多筆（Mail Merge）；圖片欄位只支援 https 網址；對方網站不允許跨網域讀取或載入失敗時，該圖會被略過並提示
...
不需建置工具：經 PHP 伺服器在瀏覽器開啟 `tests/` 下的頁面，標題會顯示 PASS／FAIL。

- `index.html`：核心（`.ptan` 讀寫、元素模型、圖文段落、渲染邊界）
