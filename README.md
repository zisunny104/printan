...
**多頁（Frame）畫布：** 工作區以 2D 方式同時排出所有頁面，直接對應實際列印結果——水平並排的是不同段（段與段之間
會切紙，以虛線＋剪刀標示），同一段內 `cutAfter:false` 串起來的頁面垂直相接（同一張連續紙）。只有目前作用中的頁面
可以直接編輯，其餘頁面是唯讀縮圖，點一下切換；縮放、尺規、「符合寬度」都以作用中頁面為準。窄螢幕（<768px）不排
2D 佈局，一次顯示一頁、用前後箭頭換頁。頁面列在「版面結構」樹的最上層（比照 Figma 圖層面板），各層可收合。

**待實機驗證（多頁列印）：** 多頁依序送出 ESC/POS 列印工作、每頁各自的切紙指令（`cutAfter`）是依 ESC/POS
規格與既有單頁列印邏輯（`buildEscposJob` 的 `cutPaper` 參數）推論出來的，開發環境沒有實體印表機，尚未
實機驗證多頁連續列印／局部切紙的實際效果。
