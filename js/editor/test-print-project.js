...
// 兩個數量沒有藏梗（純粹隨手寫的數字），改成跟當下時間掛勾，讓每次列印的金額都不一樣，方便測印時肉眼確認是新的一張：
// 「所見即所得預覽」份數＝今天幾號（1～31）；「續命美式咖啡」份數＝現在幾點（0 點算 24 杯，避免出現 0 杯）。
const menuItems = () => {
    const now = new Date();
    const previewCount = now.getDate();
    const coffeeCount = now.getHours() || 24;
    return [
        ["所見即所得預覽", previewCount, 42, "42：生命、宇宙與一切的答案；份數＝今天幾號"],
        ["續命美式咖啡", coffeeCount, 95, "95：一杯美式咖啡平均咖啡因量(mg)；份數＝現在幾點"],
