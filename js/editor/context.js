...
export const state = {
    ...
    printPrefs: { feedLines: 4, cutPaper: true, serialBaudRate: 9600, connectMethod: "usb", printableDots: {}, margins: {} }, // 走紙／切紙／序列傳輸速率／上次選的連線方式／各紙寬「可列印點數」覆寫（{ 紙寬id: 點數 }，空物件＝全用內建規格值）／各紙寬左右邊距校正（{ 紙寬id: { leftMm, rightMm } }，空物件＝不校正）偏好，跟印表機連線一樣是本機操作習慣，不進 .ptan 文件；切紙預設開啟（大多數熱感印表機使用情境都希望列印完直接切下來）。
